...
try:
    while True:
        ...
        while amount_received < amount_expected:
            ...
            mess = data.decode()
            if "WELCOME" in mess:
                print('CLIENT RECIEVED: "%s"' % mess)
                print("You are in the Lobby, the game will start soon\n")
                welcome = True
                break           
            elif "START" in mess:
                print('CLIENT RECIEVED: "%s"' % mess)
                print("Round has begun, you have 30s to make your move")
                player_lives = int(mess[-1])
                welcome = False
                break
            elif "FAIL" in mess:
                print('CLIENT RECIEVED: "%s"' % mess)
